...
class User extends CI_Controller
{
    ...
    private $venue_options = array(
        ...
        '3' => 'Instituto Tecnológioco De Leon',
        '4' => 'Instituto Tecnológico De Orizaba',
        '5' => 'Instituto Tecnológico De Zacatecas',
        '6' => 'Universidad Autónoma De Yucatán',
        '7' => 'Universidad Católica Del Norte',
        '8' => 'Universidad De Atacama',
        '9' => 'Universidad Politécnica De Aguascalientes',
        '10' => 'Universidad Politécnica De Zacatecas',
        '11' => 'Universidad Veracruzana',
        '12' => 'Centro De Bachillerato Tecnológico Industrial Y De Servicios No.168',
        '13' => 'Centro De Investigación En Matemáticas, A.C. Unidad Aguascalientes',
        '14' => 'Centro De Investigación En Matemáticas, A.C. Unidad Guanajuato',
        '15' => 'Centro De Investigación En Matemáticas, A.C. Unidad Zacatecas',
        '16' => 'Centro Universitario de Ciencias Exactas e Ingenierías',
        '17' => 'Centro Universitario de Ciencias Económico-Administrativas',
        '18' => 'Centro Universitario de los Valles',
        '19' => 'Centro Universitario de Tonalá'
    ...
    public function register($admin = "")
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // Valida si el capcha enviado desde principal.php es valido.
            // para validar, se envia el capcha con la key a la api de google
            // si es valido continua el registro, si no retorna y recarga la pagina
            $post = $this->input->post("g-recaptcha-response");
            $validate = $this->validateCapcha($post);
            $isValid = json_decode($validate)->success;
            if (!$isValid) {
                echo "<script>alert('An error has occurred, please try again later.'); window.history.back();</script>";
                return false;
            }
            ...
            }else if($registre_porfile == "13") { // si es asistente virtual
                $conference_dinner = $this->input->post("cb4", 0);
                $social_program = $this->input->post("cb5", 0);
                $additional_documentation = $this->input->post("cb6", 0);
                $IVA = $this->input->post("cb11", 0);
                //Agregamos por default el pago por ser asistente virtual
                $this->Service_model->add_user_service("12", 1, $order_id);
            ...
            $this->postageapp->subject('Register CIMPS');
    ...
    function validateCapcha($res)
    {
        $url = "https://www.google.com/recaptcha/api/siteverify"; // api de google para validar capcha

        $secret = "6Lcjz9YhAAAAAIQOhb9XNEv9Mxs87KsUT99x3n-7"; // llave secreta proporcionada por google al registar conferencecimps
        $response = $res; // token del capcha a validar
