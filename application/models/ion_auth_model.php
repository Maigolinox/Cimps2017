...
class Ion_auth_model extends CI_Model
{
	...
	public function salt()
	{
        ...
        {
                $query = $this->db->query("select nombre from downloads where id={$id}");
                $downloads = $query->row();

                return strtoupper($downloads->nombre);
        }
	...
	{
		$this->trigger_events('control_num_check');
		...
		return $this->db->where('control_num', $control_num)
		                ->count_all_results($this->tables['users']) > 0;
	}
